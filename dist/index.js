...
class CompressionPlugin {
  constructor(options = {}) {
    ...
    algorithms.forEach(({
      ...
      if (typeof algorithm === "string") {
        ...
        this.options.algorithms[index].compressionOptions = { ...defaultCompressionOptions,
          ...compressionOptions
        };
        this.options.algorithms[index].filename = filename;
      }
  ...
  runCompressionAlgorithm(algorithm, compressionOptions, input) {
    return new Promise((resolve, reject) => {
      this.algorithmFns[algorithm](input, compressionOptions, (error, result) => {
        if (error) {
          return reject(error);
        }
      ...
      const assetsForMinify = (await Promise.all(Object.keys(assets).map(async name => {
        ...
        if (typeof algorithm === "function") {
          if (typeof filename === "function") {
            relatedName = `compression-function-${_crypto.default.createHash("md5").update((0, _serializeJavascript.default)(filename)).digest("hex")}`;
          } else {
        ...
        scheduledTasks.push((async () => {
          ...
          if (!output.source) {
            if (!output.compressed) {
              try {
                output.compressed = await this.runCompressionAlgorithm(algorithm, compressionOptions, buffer);
              } catch (error) {
